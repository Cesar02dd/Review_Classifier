...
class EDA:
    ...
    def plot_distributions(self):
        ...
        """
        num_cols = len(self.data_loader.data.columns)
        fig, axes = plt.subplots(num_cols, 1, figsize=(10, 5 * num_cols))
        for i, feature in enumerate(self.data_loader.data.columns):
            sns.histplot(data=self.data_loader.data, x=feature, ax=axes[i])
    ...
    def plot_correlation_heatmap(self):
        ...
        data_with_labels = pd.concat([self.data_loader.data, self.data_loader.labels], axis=1)
    ...
    def plot_feature_importance(self, n_estimators=5, n_repeats=2):
        ...
        clf.fit(self.data_loader.data, self.data_loader.labels)

        # Compute permutation importance
        result = permutation_importance(clf, self.data_loader.data, self.data_loader.labels,
                                        n_repeats=n_repeats)
        ...
        plt.yticks(range(len(sorted_idx)), self.data_loader.data.columns[sorted_idx])
